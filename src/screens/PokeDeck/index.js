...
function App() {
      ...
      <>
        {pokedex.map(pokemon => (
          <div className="pokemon" key={pokemon.id}>
            <img alt="Pokemon" src={"https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/" + pokemon.id + ".png"} className="spritea" />
        <Link style={{ fontSize: '12px', color: '#333' }} to={{ pathname: `/pokemon/${pokemon.name}`, query: { backUrl } }}>
  ...
  function EmptyDex() {
    return (
      <p className="empty">Você ainda não possui nenhum pokémon :(</p>
    );
  }
  ...
  return (
    <div className="app-wrapper">
      ...
      <section className="wild-pokemon">
        ...
        <Link style={{ fontSize: '12px', color: '#fff' }} to={{ pathname: `/pokemon/${wildPokemon.name}`, query: { backUrl } }}>
